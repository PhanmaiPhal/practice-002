...
import org.example.ide2markingapi.init.MediaUtil;
...
public class MediaServiceImpl implements MediaService {
    ...
    @Override
    public MediaResponse uploadingSingle(MultipartFile file, String folderName) {

        //Generate new unique name for file upload
        String newName = UUID.randomUUID().toString();

        //Extract extension from file upload
        int lastDoIndex = file.getOriginalFilename()
                .lastIndexOf(".");
        String extension = file.getOriginalFilename()
                .substring(lastDoIndex + 1);
    ...
    public List<MediaResponse> uploadMultiple(List<MultipartFile> files, String folderName) {
        ...
        files.forEach(file -> {
            MediaResponse mediaResponse = this.uploadingSingle(file, folderName);
    ...
    public MediaResponse loadMediaByName(String mediaName, String folderName) {
        ...
        try {
            ...
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR,"Media has not been found!");
        }
    }
    @Override
    public MediaResponse deleteMediaByName(String mediaName, String folderName){
        ...
        try{
           ...
           throw new ResponseStatusException(HttpStatus.NOT_FOUND,
                   "Media has not been fount!");
        } catch (IOException e) {
            throw new ResponseStatusException(
                    HttpStatus.NOT_FOUND,
                    String.format("%s%s", e.getLocalizedMessage(), "can't be delete")
            );
        }
    }
